...
NUM_EXAMPLES_PER_EPOCH_FOR_EVAL = 9000
...
def inputs(eval_data, data_dir, batch_size):
  ...
  """
    if not eval_data:
        ...
        filenames = [os.path.join(data_dir, 'cifar-10-batches-py', 'test_batch')]
...
def getbatchdata(filename,batch_size):
    filename_queue = tf.train.string_input_producer([filename], capacity=3000, shuffle=True)  # 生成一个queue队列

    reader = tf.TFRecordReader()
    _, serialized_example = reader.read(filename_queue)  # 返回文件名和文件
    features = tf.parse_single_example(serialized_example,
                                       features={
                                           'label': tf.FixedLenFeature([48], tf.int64),
    ...
    # this place can operate distorted
    images, labels = tf.train.shuffle_batch([img, label], batch_size=batch_size, num_threads=2, capacity=3000 + 3 * 10,
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    images_train, labels_train = getbatchdata('../data/trainlab.tfrecords', FLAGS.batch_size)
    images_val, labels_val = getbatchdata('../data/trainlab.tfrecords', FLAGS.batch_size)
    ...
    logits = inference(images,
                             ...
                             num_blocks=[3, 4, 6, 3])
    print(logits)
    train(is_training, logits=logits, images=images, labels=labels)
